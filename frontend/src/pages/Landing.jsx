...
export default function Landing() {
  ...
  return <div className="public-site"><a className="skip-link" href="#public-main">Skip to main content</a><PublicHeader/><main id="public-main">
    <section className="landing-hero"><div><div className="public-kicker"><span/> LAND ACQUISITION · DECISION SUPPORT</div><h1>See the blockers.<br/><em>Move projects forward.</em></h1><p className="hero-intro">One workspace for acquisition records, administrative bottlenecks, and district comparisons. Built for the officials and agencies responsible for moving projects forward.</p><div className="public-actions"><Link className="button primary" to="/login">Sign in to the prototype <ArrowRight size={17}/></Link><a className="button" href="#workspace">Explore the workflow</a></div><p className="public-caption">Hackathon prototype · Illustrative records · Not an official government service</p></div>
    ...
    <section {...workspaceReveal} id="workspace"><div className="public-section-heading"><div><p className="eyebrow">ONE PORTAL, DISTINCT PURPOSES</p><h2>The right view for each task.</h2></div><p>Start with priorities. Investigate a pattern. Open the records behind it.</p></div><div className="public-cards">{[[ClipboardList,'Dashboard','What needs my attention?','Portfolio indicators, administrative bottlenecks, and a project queue.'],[ChartNoAxesCombined,'Analytics','Where are the differences?','District comparisons and acquisition-stage distributions, with filters that carry into project records.'],[FolderOpen,'Projects','What is happening on this project?','Searchable records, quick inspection, acquisition progress, and project updates.']].map(([Icon,title,question,copy]) => <button key={title} aria-pressed={view === title} onClick={()=>preview(title)}><Icon size={23}/><h3>{title}</h3><strong>{question}</strong><p>{copy}</p><span className="public-card-action">Try this view <ArrowRight size={14}/></span></button>)}</div></section>
