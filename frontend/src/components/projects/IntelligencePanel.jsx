...
function StageOutlookStrip({ rows = [] }) {
  if (!rows.length) return null;
  return <section className="stage-outlook-panel">
    <div className="advanced-ai-heading"><Layers3 size={18} /><div><h3>Stage-wise delay outlook</h3><p>Transparent screening estimates across the acquisition lifecycle. These are not calibrated stage-specific ML probabilities.</p></div></div>
    <div className="stage-outlook-grid">{rows.map(item => <article key={item.stage} className={`stage-outlook-card ${(item.risk_label || 'low').toLowerCase()} ${item.status.toLowerCase()}`}>
      <div><span>{item.stage.replaceAll('_',' ')}</span><small>{item.status}</small></div>
      <strong>{item.screening_score}/100</strong>
      <b>{item.risk_label}</b>
      <p>{item.top_drivers?.length ? item.top_drivers.join(' · ') : 'No dominant recorded driver'}</p>
    </article>)}</div>
  </section>;
}
...
export default function IntelligencePanel({ project }) {
  ...
  const [bhoomi, setBhoomi] = useState(null);
  const [stageOutlook, setStageOutlook] = useState([]);
  const [bhoomiError, setBhoomiError] = useState('');
  ...
  useEffect(() => {
    let cancelled = false;
    setBhoomi(null); setBhoomiError('');
    api(`/projects/${projectId}/bhoomirashi-predict`)
      .then(data => { if (!cancelled) setBhoomi(data); })
      .catch(err => { if (!cancelled) setBhoomiError(err.message); });
    api(`/projects/${projectId}/stage-outlook`)
      .then(data => { if (!cancelled) setStageOutlook(Array.isArray(data) ? data : []); })
      .catch(() => { if (!cancelled) setStageOutlook([]); });
  ...
  return <SectionCard
    ...
    description="One decision view: acquisition condition → predictive evidence → accountable intervention."
  ...
  >
    ...
    {!prediction && <StageOutlookStrip rows={stageOutlook}/>}
    ...
    {prediction ? <>
      ...
      <div className="intelligence-section-head">
        <div><span className="eyebrow"><Activity size={14} /> PREDICTIVE EVIDENCE</span><h3>Two separate model signals</h3><p>PAIMANA is the validated wider-project early warning. Bhoomi Rashi is an experimental acquisition-stage research signal.</p></div>
      </div>

      <div className="predictive-evidence-grid">
        <section className="ml-support-card paimana-support-card">
          ...
          <p>Real longitudinal MoSPI project histories. Independent from the acquisition operational fields shown above.</p>
          <div className="ml-support-meta"><span>{prediction.metadata.data_period}</span><span>3-month horizon</span><span>{prediction.metadata.calibration_method || 'Raw probability'}</span></div>
          <div className="evidence-card-foot"><span><ShieldAlert size={14} /> Wider-project schedule evidence</span><small>Does not use compensation, possession, R&amp;R, approvals or disputes.</small></div>
        </section>

        <section className="ml-support-card bhoomi-support-card">
          <div className="ml-support-heading"><div><span className="eyebrow"><Database size={14} /> EXPERIMENTAL ACQUISITION ML</span><h3>Bhoomi Rashi stage-delay signal</h3></div>{bhoomi?.available ? <div className={`ml-signal-pill ${(bhoomi.label || 'low').toLowerCase()}`}><strong>{Math.round((bhoomi.probability || 0) * 100)}%</strong><span>{bhoomi.label}</span></div> : <div className="ml-signal-pill"><strong>—</strong><span>UNAVAILABLE</span></div>}</div>
          {bhoomi?.available ? <>
            <p>Research probability that a chronology-matched 3A→3D interval exceeds 365 days.</p>
            <div className="bhoomi-coverage"><div><span>Current input coverage</span><strong>{bhoomi.input_coverage_pct}%</strong></div><div className="coverage-track"><span style={{ width: `${Math.max(0, Math.min(100, bhoomi.input_coverage_pct || 0))}%` }} /></div></div>
            <div className="bhoomi-compact-metrics"><span><small>ROC-AUC</small><strong>{bhoomi.mean_roc_auc?.toFixed(3)}</strong></span><span><small>Recall</small><strong>{bhoomi.mean_recall?.toFixed(3)}</strong></span><span><small>Projects</small><strong>{bhoomi.unique_projects}</strong></span></div>
            <details className="bhoomi-method"><summary>Research limitations &amp; feature mapping</summary><p>{bhoomi.disclaimer}</p><p><strong>Proxy mappings:</strong> {bhoomi.proxy_fields?.join(' · ')}</p><p><strong>Imputed features:</strong> {bhoomi.imputed_fields?.join(', ')}</p><p><strong>Training set:</strong> {bhoomi.dataset_rows?.toLocaleString()} cleaned observations · {bhoomi.validation}</p></details>
          </> : <p>{bhoomiError || bhoomi?.disclaimer || 'Experimental Bhoomi Rashi signal is currently unavailable.'}</p>}
        </section>
      </div>


      <StageOutlookStrip rows={prediction.stage_outlook || stageOutlook}/>

      <div className="intervention-launch-strip intervention-launch-strip-v2"><div><Sparkles size={20} /><span><strong>Move from evidence to action</strong><small>Simulate administrative actions against acquisition risk, readiness and intervention priority before tracking the selected action.</small></span></div><button className="button primary" onClick={() => setSimulatorOpen(true)}>Open Intervention Simulator</button></div>
