...
// One role-choice button, with its own pointer-tracked spotlight and a staggered
// entrance delay (set via --index) so the four options settle in one after another
// rather than as a single block.
function RoleChoice({ icon: Icon, title, selected, index, onClick }) {
  const spotlight = useSpotlight();
  return <button {...spotlight} style={{ '--index': index }} aria-pressed={selected} onClick={onClick}>
    <Icon size={20}/><span>{title}</span>{selected ? <Check size={16}/> : <ArrowRight size={16}/>}
  </button>;
}
...
export default function RoleJourney({audiences}) {
  ...
  return <div className="role-explorer"><div className="role-choices" aria-label="Choose your role">{audiences.map(([Icon,title],i)=><RoleChoice key={title} icon={Icon} title={title} selected={selected===i} index={i} onClick={()=>setSelected(i)}/>)}</div><div className="role-journey" key={selected} aria-live="polite"><p className="eyebrow">YOUR PROPOSED WORKFLOW</p><h3>{journey.title}</h3><ol>{journey.tasks.map(task=><li key={task}>{task}</li>)}</ol><p className="role-scope">{journey.scope}</p><Link className="button" to={journey.route ? '/login' : '/access'} state={journey.route ? {from:journey.route} : undefined}>{journey.route ? 'Sign in to the relevant view' : 'Explore access guidance'} <ArrowRight size={15}/></Link><small>Selecting a role here explains the workflow; it does not grant permissions.</small></div></div>;
}
